...
and each step of the automation can be clicked and viewed in detail. It traces the complete run of an automation run.
...
So imagine this little template: `{% raw %}{{ my_variable }}{% endraw %}`
...
frontend, you can click on the "CONFIGURE DEVICE" button that is shown on each
...
Please note, these new template sensors are configured under the `template:`
key in the configuration and are using a new configuration format and keys.
...
You can now use UI Selectors for your script field parameters. This
brings the same UI capabilities that Blueprints have to scripts. This allows for
...
- [Panasonic Viera][panasonic_viera docs] now has support for remotes, added by [@joogps]
...
New format is "2021-03-20T10:35:47Z". This allows automations to use their
value based on `as_timestamp()`.

If you rely on these attributes in your automation or script, you might need
...
Note, the existing configuration style will be removed in a couple of releases.
...
a future release of Home Assistant. The integration has used a configuration via
